...
- Format: `uv run ruff format --check plugins/ai-agents/hooks scripts` — must succeed.
- Types: `uv run ty check --extra-search-path plugins/ai-agents/hooks --extra-search-path scripts` — must succeed (Astral ty; CLI extra paths required so PEP 723 entrypoints resolve sibling imports).
- Tests: `uv run pytest` — must succeed (tests live under repo `tests/`, not inside the shipped plugin).
...
- Run the full Validate suite (plugin, lint, format, types, tests, dependency audit) at review / commit / PR milestones.
