...
**Bump vs grouping:** the **branch prefix** (`feature/`, `fix/`, …) selects the version bump — see `skills/ankyr-ci-worktree/references/branch-naming.md`. The **PR title** selects the release-notes group under `summary_mode: header_only`. These are different jobs; do not drop the title prefix just because the branch already names the bump type.
