...
1. **Require explicit push consent.** Never push without it (see `rules/behaviors/git-push-consent.mdc`). General work still needs an explicit "push" request. **Exception:** when running `pr-resolver`, approving the resolver plan (or an explicit "resolve comments" request) grants scoped commit+push consent for approved fix commits only — see pr-resolver `## Scoped consent`.
