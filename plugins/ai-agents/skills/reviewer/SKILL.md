...
Load `skills/engineering/SKILL.md` first — Phase 0 runs its checks on every tier before any domain phase.
...
| `*.{py,pyi}` | + `python` |
| `test_*.py`, `*_test.py`, `*.{test,spec}.{ts,tsx,js}`, `conftest.py` | + `testing` + the matching language skill's `references/testing.md` |
| `*.{ts,tsx,js,mjs}` | + `nodejs` |
| `*.{tsx,jsx}` components/pages | + `react`, `ui`, `ux` |
| `**/three/**`, shaders, R3F/WebGL | + `threejs`, `performance` |
| effects/timers/listeners/audio/GPU | + `performance` |
| auth / input / data / secrets | + `security` |
| GSAP / animation / VFX UI | + `react` (gsap-patterns), `ux` |
...
| 2 | React structure | `react` + frontend lens |
| 3 | UI / a11y | `ui` + frontend lens |
| 3b | UX / interactivity | `ux` + frontend / motion-vfx lenses |
| 4 | Performance / memory | `performance` (+ realtime-graphics when 3D) |
| 5 | Security | `security` (+ backend lens when data/auth) |
