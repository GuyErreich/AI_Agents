...
An effect callback cannot be async. Wrap async work in a `void`-prefixed IIFE with `try/catch` (see `nodejs` async-await reference).

## Cleanup

Every effect that creates a resource (listener, timer, subscription, context, GPU resource) must return a cleanup that releases it. See `performance` for the catalog.
