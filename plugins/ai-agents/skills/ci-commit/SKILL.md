...
3. **Review at change tier.** Run `skills/reviewer/SKILL.md` (tier: change) on the **full** working-tree change set. If findings exist, run the local review loop (`skills/ci-local-review-loop/SKILL.md`) until the verdict is clean or the user explicitly skips. Record the verdict if using the lockfile.
...
Use the same subject-line prefixes as PR titles — see `skills/ci-pr/references/title-conventions.md` (`feat:`, `fix:`, etc.).
