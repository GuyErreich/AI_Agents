...
Illustrations of container-tier and aggregation decisions. Folder nesting still follows `hierarchy`.

## 1. Quality vs language — deepen vs mislabel

**Candidates:** `performance`, `security` under `nodejs`.
...
**Now:** `skills/ankyr-improvement-protocol` (and this `ankyr-agent-hierarchy` skill).
...
## 4. `ux` references — subfolders are fine, chains are not

**Candidate:** `ux/references/{desktop,mobile,shared}/` plus two top-level references.
...
**Candidates:** `testing/references/python-pytest.md` vs `python/references/testing.md` (and the Node twin).
