---
name: ankyr-agent-hierarchy
description: Agent-library hierarchy — when to add a rule, SKILL.md, reference, or asset; link depth vs folder depth; prefer deepening over widening. Use when creating or reorganizing skills, rules, or AGENT.md under .cursor/ or the agent stack.
...
Resolve standards project-first (`AGENT.md` chain, project rules, project skills, then this plugin if installed). Preferred if installed: hierarchy first. Nesting and retrieval tests apply; this skill adds **container tiers** and agent-specific aggregation.
...
- Cross-skill pointers (`see skills/ankyr-react/references/gsap-patterns.md`) are "see also" hints, not load paths — the owning skill still links its own references directly.

## No hard cross-skill loads

Skills must not require another skill file as a hard first step (`## Extends` + `Load skills/<name>/SKILL.md first`). Cursor does not load files across plugins, and a consuming project may already own that capability.

- Open with `## Foundation` and a one-line resolution pointer (`AGENT.md` → project rules → project skills → this plugin if installed).
- Domain orderings (for example react after nodejs) are **preferred if installed**, never required.
- Cross-skill pointers remain "see also" hints, not load paths.

## Plugin skill discovery

Plugin skills must live at **one level** under the skills root: `skills/<name>/SKILL.md`, and frontmatter `name` must equal that folder. Cursor's plugin loader discovers each subdirectory of `skills/` that contains `SKILL.md`; nested trees such as `skills/code/languages/python/` do not register. Category grouping belongs in the skill `name` (`ankyr-ci-commit`, `ankyr-agent-hierarchy`), not extra folders.
...
Then apply the nesting and retrieval tests from `hierarchy` to the chosen path.
...
| Two skills with identical `Foundation` pointers always loaded together | Merge |
