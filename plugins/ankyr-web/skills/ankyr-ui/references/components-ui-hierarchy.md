...
- [ ] Generative sound / Framer adoption? → nearest AGENT.md + ux → generative-sound.md
```
...
- Structure / placement → `ui` → `components-ui-hierarchy.md`
- Mobile variant feel → `ux` + `mobile/*` refs (via `mobile-ux` rule)
- Desktop variant feel → `ux` + `desktop/*` refs (via `desktop-ux` rule)
- Generative sound adoption → nearest AGENT.md + `ux` → `generative-sound.md`
```
...
Feature layout variants are not a PWA app shell — see `ux/references/viewport-routing.md`.
