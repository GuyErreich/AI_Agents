...
Rule pairing table: `ui` → `components-ui-hierarchy.md` → **Rule pairing**.
...
## Portable glob rules (`rules/ankyr/web/`)

| Rule | Glob | Role |
|---|---|---|
| `ui.mdc` | `**/*.{tsx,jsx,css,scss}` | Structure → `ui` |
| `ux.mdc` | `**/*.{tsx,jsx,css,scss}` | Generic feel → `ux` |
