---
name: ankyr-security
description: Vulnerability prevention — injection, XSS, CSRF, auth/session, input validation, secret handling, uploads, and safe error logging. Use when code touches user input, auth, API routes, data access, or sensitive data.
...
Resolve standards in this order: the `AGENT.md` chain (leaf → root), then project rules, then project skills, then this plugin's matching skill if installed. The first source that speaks wins.

## Non-negotiable floor

These floors always apply. A later source in the resolution order cannot waive them:

- Never commit or push secrets, credentials, or private keys.
- Never log secrets or PII.
- Never expose server-only secrets to the client.
- Never interpolate untrusted input into a query, command, or HTML sink.
- Consent gates (commit, push) and the code-review gate remain in force.
