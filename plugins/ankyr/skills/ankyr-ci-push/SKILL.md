...
1. **Require explicit push consent.** Never push without it (see `rules/ankyr/behaviors/git-push-consent.mdc`). General work still needs an explicit "push" request. **Exception:** when running `ankyr-pr-resolver`, approving the resolver plan (or an explicit "resolve comments" request) grants scoped commit+push consent for approved fix commits only — see pr-resolver `## Scoped consent`.
...
4. **If an open PR exists**, fetch its review threads (see `pr-resolver` graphql reference). If there are unresolved threads, stop and hand off to pr-resolver — do not push unrelated changes on top. **Exception:** pr-resolver may push after validation when executing an approved fix plan (its Step 6).
