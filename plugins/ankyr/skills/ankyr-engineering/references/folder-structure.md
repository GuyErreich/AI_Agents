...
For **domain-agnostic** placement (nesting vs hoist, one axis per level, restructure when trees drift), load `hierarchy` first. This file keeps only the **code** application of those principles.
...
The exact folder names (for example `src/lib/`, `src/hooks/`, `common/`, `desktop/`, `mobile/`) are project conventions. Read the nearest `AGENT.md` and the `ui` skill for the concrete layout in this repository.
