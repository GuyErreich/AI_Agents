...
1. Prefer the project's reviewer; if none, load this plugin's `skills/ankyr-reviewer/SKILL.md` if installed, at **pr** tier (`merge-base...HEAD` vs base from `AGENT.md`).
...
3. Capability routing for paths in scope: project standards first, then this plugin's defaults if installed. Nearest `AGENT.md` per path. Findings must trace to a standard that applies to this project.
...
5. Load `skills/ankyr-reviewer/references/analysis-modes.md` when `analysis_mode` is present (always preferred). Apply the mode's finding quality bar for this pass.
6. Round focus:
...
2. Project standards, then engineering default → matching lenses → domain providers if installed → logic → threat → validate (or skip) → coverage gate.
