...
2. Prefer the project's resolver; if none, load this plugin's `skills/ankyr-pr-resolver/SKILL.md` if installed, for reply/resolve mechanics only. **Skip** its interactive Plan-mode gate — the orchestrator already triaged.
...
4. Before editing a file, read it and the nearest `AGENT.md`. Resolve standards project-first, then load matching domain providers from the reviewer capability table only as needed.
